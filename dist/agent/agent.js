...
async function runAgent(userInput) {
    ...
    let response = await openaiClient_1.openai.responses.create({
        model: "gpt-4.1",
        // input: messages,
        input: userInput,
        tools: tools_1.tools,
    ...
    while (true) {
        ...
        const tool = toolRegistry_1.toolRegistry[toolCall.name]; // puoi aggiungere nuovi tool senza cambiare l’agent loop.
        if (!tool) {
